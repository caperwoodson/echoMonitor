...
//Run with ports below 1024 with sudo: sudo node PORT=80 node  server.js
...
app.post('/echoData', function(req, res){
  ...
  if (summary.indexOf("off everything")!=-1) {
    redled.writeSync(0);
    greenled.writeSync(0);
    lamp.writeSync(0);
    fan.writeSync(0);
  }
   if (summary.indexOf("on everything")!=-1) {
    redled.writeSync(1);
    greenled.writeSync(1);
    lamp.writeSync(1);
    fan.writeSync(1)
  }
   if (summary.indexOf("blow me")!=-1) {
    fan.writeSync(1);    
